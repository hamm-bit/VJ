...
int n;
ll h1, h2;
array<array<ll, 3>, MAX> DP{};
array<array<int, 2>, MAX> Hs{};
...
int main() {
    ...
    // UPDATE: Adding a third array in case we do not take from DP.front();
    // UPDATE 2: Should be <ll> for DP
    
    DP[0][0] = 0;
    DP[0][1] = 0;
    for (int i = 0; i++ < n;) {
        DP[i][0] = max(DP[i-1][0], DP[i-1][1] + (ll) Hs[i][0]);
        DP[i][1] = max(DP[i-1][1], DP[i-1][0] + (ll) Hs[i][1]);
        // cout << DP[i][0] << " " << DP[i][1] << "\n";
    }
